...
/** external prototypes */
extern DB2ConnEntry* db2AllocConnHdl      (DB2EnvEntry* envp,const char* srvname, char* user, char* password, char* jwt_token);
extern DB2EnvEntry*  db2AllocEnvHdl       (void);
...
/** local prototypes */
DB2Session*          db2GetSession        (const char* srvname, char* user, char* password, char* jwt_token, int curlevel);
...
DB2Session* db2GetSession (const char* srvname, char* user, char* password, char* jwt_token, int curlevel) {
  ...
  /* use the cached environment handle, or create it */
  ...
  if (connp == NULL){
    connp = db2AllocConnHdl(envp, srvname, user, password, jwt_token);
  }
